...
const variants = ref([{ name: '', sku: '', stock: null, values: [{ value: '', price: '' }] }]);

const addVariant = () => {
  variants.value.push({ name: '', sku: '', stock: null, values: [{ value: '', price: '' }] });
};
...
const formData = reactive({
  ...
  galleryImages: [],
  hasVariant: false,
  variants: [{ name: '', sku: '', stock: null, values: [{ value: '', price: '' }] }],
});
        ...
        <form @submit.prevent="saveProductData" enctype="multipart/form-data">
          <div class="card-body">
            ...
            <div class="row">
              ...
              <div class="col-12" v-if="hasVariant">
                ...
                <div v-for="(variant, variantIndex) in variants" :key="variantIndex" class="mb-4">
                  ...
                  <div v-for="(value, valueIndex) in variant.values" :key="valueIndex" class="mb-2">
                    <div class="row">
                      ...
                      <div class="col-md-2">
                        <button class="btn btn-danger" @click="removeVariantValue(variantIndex, valueIndex)">Remove</button>
                      </div>
                    </div>
                  </div>
                  <button type="button" class="btn btn-primary mt-2" @click="addVariantValue(variantIndex)">Add Value</button>
                  <button type="button" class="btn btn-danger mt-2 ml-2" @click="removeVariant(variantIndex)">Remove Variant</button>
                </div>
            ...
            <div class="col-md-12 mb-3">
              <div class="form-group">
                ...
                <input type="file" name="gallery_images" class="form-control" multiple @change="onGalleryFileChange" accept="image/*" />
              </div>
            </div>
